...
export function AddScheduledTextModal({ visible, onClose, onTextScheduled, theme }: AddScheduledTextModalProps) {
  ...
  const loadProfiles = async () => {
    try {
      ...
      // Filter profiles that have phone numbers
      const profilesWithPhones = allProfiles.filter(profile => profile.phone);
      setProfiles(profilesWithPhones);
    } catch (error) {
  ...
  const setQuickDate = (days: number) => {
    ...
    if (days === 0) {
      // For "Today", set a reasonable future time
      ...
      if (currentHour >= 22) {
        ...
        const suggestedHour = Math.max(currentHour + 1, 19);
  ...
  const getSelectedProfileName = () => {
    if (!selectedProfile) return 'Select a contact';
    const profile = profiles.find(p => p.id === selectedProfile);
        ...
        <View style={[styles.modal, { backgroundColor: theme.cardBackground }]}>
          ...
          <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
            ...
            <View style={styles.section}>
              <Text style={[styles.label, { color: theme.text }]}>Contact (Optional)</Text>
              <TouchableOpacity
                ...
                <View style={[styles.profilePicker, { backgroundColor: modalTheme.inputBackground, borderColor: modalTheme.inputBorder }]}>
                  ...
                  {profiles.map((profile) => (
                    ...
                    >
                      ...
                      <Text style={[styles.profileOptionPhone, { color: modalTheme.placeholderColor }]}>
                        {profile.phone}
                      </Text>
                    </TouchableOpacity>
                  ))}
